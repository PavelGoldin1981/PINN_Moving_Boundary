...
#R_values = nn.Parameter(torch.linspace(0.05, 0.15, N_t).to(device))
...
for epoch in range(75000):
    ...
    if epoch % 5 == 0:
        print(f"Epoch {epoch}, Loss: {loss.item():.4e},  loss_ic: {loss_ic.item():.4e}, loss_R: {loss_R.item():.2e}, R0 Loss: {loss_r0.item():.2e}")
...
with torch.no_grad():
    ...
    with torch.no_grad():
        ...
        # --- Список времён, для которых хотим профили ---
        ...
        Nt = 50
        Nr = 200
        t_grid = np.linspace(0.05, 1.0, Nt)  # пропускаем совсем малые времена
        ...
        # ---- аналитический Васильев на той же сетке ----
    ...
    grad_pinn = np.zeros_like(phi_pinn)
    grad_vas = np.zeros_like(phi_vas)

    # центральные разности по r для каждой строки по времени
    for i in range(Nt):
        # середина: центральная разность
        grad_pinn[i, 1:-1] = (phi_pinn[i, 2:] - phi_pinn[i, :-2]) / (2.0 * dr)
        grad_vas[i, 1:-1] = (phi_vas[i, 2:] - phi_vas[i, :-2]) / (2.0 * dr)

        # край слева: односторонняя вперёд
        grad_pinn[i, 0] = (phi_pinn[i, 1] - phi_pinn[i, 0]) / dr
        grad_vas[i, 0] = (phi_vas[i, 1] - phi_vas[i, 0]) / dr

        # край справа: односторонняя назад
        grad_pinn[i, -1] = (phi_pinn[i, -1] - phi_pinn[i, -2]) / dr
        grad_vas[i, -1] = (phi_vas[i, -1] - phi_vas[i, -2]) / dr

    # ---- ошибка по градиенту ----
    grad_err = np.abs((grad_pinn - grad_vas)) / np.abs(grad_vas+0.001)

    # ---- тепловая карта ошибки градиента ----
    fig, ax = plt.subplots(figsize=(10, 6))
    c = ax.pcolormesh(t_grid, r_grid, grad_err.T, shading='auto')
    ax.set_xlabel(r'$\tilde{t}$')
    ax.set_ylabel(r'$\tilde{r}$')
    cb = fig.colorbar(c, ax=ax)
    cb.set_label(r'$|\partial_{\tilde r}\varphi_{\mathrm{PINN}} - \partial_{\tilde r}\varphi_{\mathrm{Vas}}|$')

    plt.tight_layout()
    plt.savefig("fig_error_grad_heatmap.pdf", bbox_inches='tight')
    plt.show()




    # --- Время, по которому сравниваем
    #t_fixed = 0.5 #0.5

    # --- Интерполированный радиус R(t_fixed)
    #R_learned_np = R_values.detach().cpu().numpy()
    #t_vals_np = t_nodes.cpu().numpy()
    #R_fixed = np.interp(t_fixed, t_vals_np, R_learned_np)

    # --- Диапазон r и соответствующие значения ρ
    #r_test = np.linspace(R_fixed, R_max, 400)
    #rho_vals = r_test / R_fixed
    #t_test = np.full_like(r_test, t_fixed)
    ...
    # --- Васильевское решение
    #phi_exact = 1 - phi_vasiliev(rho_vals, b_value)

    # --- График сравнения
    #plt.figure(figsize=(10, 5))
    #plt.plot(r_test, phi_pred, label='PINN')
    #plt.plot(r_test, phi_exact, '--', label='Vasil\'ev')
    #plt.xlabel(r'$\tilde{r}$')
    #plt.ylabel(r'$\varphi(\tilde{r}, \tilde{t}=0.5)$')
    #plt.title('PINN vs Vasil\'ev Solution at $\\tilde{t} = 0.5$')
    #plt.legend()
    #plt.grid(True)
    #plt.tight_layout()
    #plt.savefig('fig_profile_t05.pdf', bbox_inches='tight')
    #plt.show()

    #plt.savefig('fig_profile_t05.pdf', bbox_inches='tight')
    #plt.show()

    # --- Сохраняем профиль концентрации ---
    #profile_data = np.vstack([r_test, phi_pred])
    #np.save(f"profile_R0_{R_learned_np[0]:.3f}.npy", profile_data)
    #print(f"Saved concentration profile to: profile_R0_{R_learned_np[0]:.3f}.npy")
    ...
    # Сохрани в npy
    #np.save(f"R_vs_t_R0_{R_learned[0]:.3f}.npy", np.vstack([t_vals, R_learned]))
    #print(f"Saved to: R_vs_t_R0_{R_learned[0]:.3f}.npy")